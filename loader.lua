...
function IslandEsp() 
	for i, v in pairs(game:GetService("Workspace")["_WorldOrigin"].Locations:GetChildren()) do
		pcall(function()
			if _G.Save.islEsp then 
				if not v:FindFirstChild("NameEspIsland") then
					if not v:FindFirstChild('NameEspIsland') then
                        local bill = Instance.new("BillboardGui", v)
                        local esp = Instance.new("TextLabel", bill)
        
                        bill.Name = "NameEspIsland"
                        bill.ResetOnSpawn = false
                        bill.AlwaysOnTop = true
                        bill.LightInfluence = 0;
                        bill.MaxDistance = 
				...
				if v:FindFirstChild('NameEspIsland') then
					v:FindFirstChild('NameEspIsland'):Destroy()
				end
			end
		end)
	end
...
local tpIsland = tps:Dropdown{
	...
	Callback = function(v)
		wait(0.1)
		topos(game:GetService("Workspace")["_WorldOrigin"].Locations[v].Position)
		_G.Clip = true
	end
}
...
visu:Toggle{
	...
	Description = nil,
	Callback = function(state)
        _G.Save.islEsp = state
		saveSettings()
        while _G.Save.islEsp do
			wait()
            IslandEsp()
        end
	end
